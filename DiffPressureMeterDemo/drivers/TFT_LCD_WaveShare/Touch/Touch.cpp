...
{
	lcd.lcd_draw_line(hwXpos - 12, hwYpos, hwXpos + 13, hwYpos, hwColor);
	lcd.lcd_draw_line(hwXpos, hwYpos - 12, hwXpos, hwYpos + 13, hwColor);
	lcd.lcd_draw_point(hwXpos + 1, hwYpos + 1, hwColor);
	lcd.lcd_draw_point(hwXpos - 1, hwYpos + 1, hwColor);
	lcd.lcd_draw_point(hwXpos + 1, hwYpos - 1, hwColor);
	lcd.lcd_draw_point(hwXpos - 1, hwYpos - 1, hwColor);
	lcd.lcd_draw_circle(hwXpos, hwYpos, 6, hwColor);
}

void TP::tp_draw_big_point(uint16_t hwXpos, uint16_t hwYpos, uint16_t hwColor)
{
	lcd.lcd_draw_point(hwXpos, hwYpos, hwColor);
	lcd.lcd_draw_point(hwXpos + 1, hwYpos, hwColor);
	lcd.lcd_draw_point(hwXpos, hwYpos + 1, hwColor);
	lcd.lcd_draw_point(hwXpos + 1, hwYpos + 1, hwColor);
}
                     ...
                     uint16_t hwXpos3, uint16_t hwYpos3, uint16_t hwFac)
{
	...
	lcd.lcd_display_string(40, 160, (const uint8_t *)"x1", 16, RED);
	lcd.lcd_display_string(40 + 80, 160, (const uint8_t *)"y1", 16, RED);

	lcd.lcd_display_string(40, 180, (const uint8_t *)"x2", 16, RED);
	lcd.lcd_display_string(40 + 80, 180, (const uint8_t *)"y2", 16, RED);

	lcd.lcd_display_string(40, 200, (const uint8_t *)"x3", 16, RED);
	lcd.lcd_display_string(40 + 80, 200, (const uint8_t *)"y3", 16, RED);

	lcd.lcd_display_string(40, 220, (const uint8_t *)"x4", 16, RED);
	lcd.lcd_display_string(40 + 80, 220, (const uint8_t *)"y4", 16, RED);

	lcd.lcd_display_string(40, 240, (const uint8_t *)"fac is:", 16, RED);

	lcd.lcd_display_num(40 + 24, 160, hwXpos0, 4, 16, RED);
	lcd.lcd_display_num(40 + 24 + 80, 160, hwYpos0, 4, 16, RED);

	lcd.lcd_display_num(40 + 24, 180, hwXpos1, 4, 16, RED);
	lcd.lcd_display_num(40 + 24 + 80, 180, hwYpos1, 4, 16, RED);

	lcd.lcd_display_num(40 + 24, 200, hwXpos2, 4, 16, RED);
	lcd.lcd_display_num(40 + 24 + 80, 200, hwYpos2, 4, 16, RED);

	lcd.lcd_display_num(40 + 24, 220, hwXpos3, 4, 16, RED);
	lcd.lcd_display_num(40 + 24 + 80, 220, hwYpos3, 4, 16, RED);

	lcd.lcd_display_num(40 + 56, 240, hwFac, 3, 16, RED);
}

uint8_t TP::tp_scan(uint8_t chCoordType)
{
	if (!(xpt.xpt2046_read_input_pin_status())) {
		if (chCoordType) {
				...
				case 4:	
					...
					if(abs(s_tTouch.fXfac) > 2 || abs(s_tTouch.fYfac) > 2) {
						cnt=0;
 				    	tp_draw_touch_point(LCD_WIDTH - 20, LCD_HEIGHT - 20, WHITE);
						tp_draw_touch_point(20, 20, RED);								
						lcd.lcd_display_string(40, 26, (const uint8_t *)"TP Need readjust!", 16, RED);
						continue;
					}
					lcd.lcd_clear_screen(WHITE);
...
{
	lcd.lcd_clear_screen(WHITE);
	lcd.lcd_display_string(LCD_WIDTH - 40, 0, (const uint8_t *)"CLEAR", 16, BLUE);
}
