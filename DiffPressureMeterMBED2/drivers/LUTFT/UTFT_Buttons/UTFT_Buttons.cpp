...
UTFT_Buttons::UTFT_Buttons(LUTFT *ptrUTFT, LUTouch *ptrURTouch)
{
	_UTFT = ptrUTFT;
	_URTouch = ptrURTouch;
	deleteAllButtons();
...
void UTFT_Buttons::enableButton(int buttonID, bool redraw)
{
...
void UTFT_Buttons::disableButton(int buttonID, bool redraw)
{
	if (!(buttons[buttonID].flags & BUTTON_UNUSED))
	{
		buttons[buttonID].flags = buttons[buttonID].flags | BUTTON_DISABLED;
...
bool UTFT_Buttons::buttonEnabled(int buttonID)
{
